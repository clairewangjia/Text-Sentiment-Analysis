...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, lexicon_file="sentiment_lexicon.txt"):
        ...
        emoji_lexicon = {}
        with io.open(os.path.join(dirname(_this_module_file_path_), 'emoji_lexicon.txt'), encoding='utf-8') as f:
            lis = [line.split() for line in f]
            for i,row in enumerate(lis):
                emoji = 'e_' + ''.join(row).strip().split(',')[8]
                measure = ''.join(row).strip().split(',')[7]
                emoji_lexicon[emoji] =  float(measure)*4
        self.lexicon = dict(self.lexicon.items() + emoji_lexicon.items())
...
def main():
    ...
    for train, test in kf.split(y):
        ...
        for instance in x[test]:
            score_dict = classifier.polarity_scores(instance)
            neg = score_dict["neg"]
            neu = score_dict["neu"]
            pos = score_dict["pos"]
            compound = score_dict["compound"]
            predict_scores.append(compound)
            predict_prob.append([neg, neu, pos])
        prob_text = np.concatenate((prob_text, np.array(predict_prob)),axis=0)
    ...
    print('Average Precision of vader_sentiment_analyzer is %f.' %(avg_p/10.0))
    print('Average Recall of vader_sentiment_analyzer is %f.' %(avg_r/10.0))
    print('Average Macro-F1 of vader_sentiment_analyzer is %f.' %(macro_f1/10.0))
    return prob_text
